...
# FIXME: GPU tests need this to pass for some reason.
#   https://github.com/flatironinstitute/jax-finufft/issues/162
x32_rtol = 2e-6
...
class TestFastInterp:
    ...
    def test_non_uniform_real_FFT_2D(self, func, m, n, domain_x, domain_y):
        ...
        g = true_g(xq, yq)
        np.testing.assert_allclose(g1(xq, yq), g, atol=5e-11, rtol=x32_rtol)
    ...
    def test_mask(self):
        ...
        f = jnp.ones((7, 7), dtype=jnp.complex128)

        x = jnp.array([0.1, 0.5, 1.2, 3.1, 4.5, 5.9])
        y = jnp.array([0.2, 0.6, 1.3, 3.2, 4.6, 6.0])
        mask = jnp.array([True, False, True, False, True, False])

        out = nufft2(f, x, y, points_mask=mask)
        np.testing.assert_array_equal(out[~mask], 0.0)
        np.testing.assert_allclose(
            out[mask], nufft2(f, x[mask], y[mask]), rtol=x32_rtol
        )
