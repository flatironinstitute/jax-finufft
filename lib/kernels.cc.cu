...
__global__ void unpack_kernel(const int8_t* mask, const int64_t* prefix, int64_t n_j,
  ...
  if (i < n_j) {
    if (mask[i]) {
      ...
      for (int t = 0; t < n_transf; t++) c[i * n_transf + t] = {0, 0};
    }
  }
}
...
ffi::Error run_nufft_masked(cudaStream_t stream, cufinufft_opts opts, T eps, int iflag,
  ...
  for (int64_t index = 0; index < n_tot; ++index) {
    ...
    pack_kernel<<<blocks, threads, 0, stream>>>(mask + i_start, d_prefix, n_j, &x[i_start],
                                                ndim > 1 ? &y[i_start] : nullptr,
                                                ndim > 2 ? &z[i_start] : nullptr, &c[c_start], Q_x,
                                                Q_y, Q_z, Q_c, n_transf, type);
    ...
    if constexpr (type == 2) {
      unpack_kernel<<<blocks, threads, 0, stream>>>(mask + i_start, d_prefix, n_j, Q_c, &c[c_start],
                                                    n_transf);
    }
