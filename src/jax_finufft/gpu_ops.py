...
from .jax_finufft_cpu import build_descriptor, build_descriptorf
...
def cunufft1(output_shape, source, *points, iflag=1, eps=1e-6):
    iflag = int(iflag)
    eps = float(eps)
    ndim = len(points)
    if not 1 <= ndim <= 3:
...
def cunufft2(source, *points, iflag=-1, eps=1e-6):
    iflag = int(iflag)
    eps = float(eps)
    ndim = len(points)
    if not 1 <= ndim <= 3:
...
def translation_rule(
    ...
    desc = globals()[f"build_descriptor{suffix}"](
...
xla.register_translation(nufft1_p, partial(translation_rule, 1), platform="CUDA")
...
xla.register_translation(nufft2_p, partial(translation_rule, 2), platform="CUDA")
